...
def run_tier_a() -> dict[str, int]:
    """Tier A — Prose (router -> switch in docs)."""
    counts = {"prose_router_to_switch": 0, "file_renames": 0}
...
def run_tier_b() -> dict[str, int]:
    """Tier B — Identifiers & Code Modules (router -> switch, adapter -> egress/ingress)."""
    ...
    old_cli_adapter = REPO_ROOT / "src" / "flock" / "adapter" / "cli.py"
    new_egress_adapter = REPO_ROOT / "src" / "flock" / "adapter" / "egress_adapter.py"
    if old_cli_adapter.exists():
        if new_egress_adapter.exists():
            new_egress_adapter.unlink()
        old_cli_adapter.rename(new_egress_adapter)
        counts["file_moves"] += 1
        counts["adapter_renames"] += 1

    old_runner_adapter = REPO_ROOT / "src" / "flock" / "adapter" / "runner.py"
    new_ingress_adapter = REPO_ROOT / "src" / "flock" / "adapter" / "ingress_adapter.py"
    if old_runner_adapter.exists():
        if new_ingress_adapter.exists():
            new_ingress_adapter.unlink()
        old_runner_adapter.rename(new_ingress_adapter)
        counts["file_moves"] += 1
        counts["adapter_renames"] += 1
    ...
    adapter_patterns = [
        ("from .runner import run_adapter", "from .ingress_adapter import run_adapter"),
        ("from flock.adapter.cli", "from flock.adapter.egress_adapter"),
        ("from flock.adapter.runner", "from flock.adapter.ingress_adapter"),
        ("from flock.adapter import runner", "from flock.adapter import ingress_adapter"),
        ("import flock.adapter.runner", "import flock.adapter.ingress_adapter"),
        ("flock.adapter.runner", "flock.adapter.ingress_adapter"),
        ("flock.adapter.cli", "flock.adapter.egress_adapter"),
        ("hasattr(runner,", "hasattr(ingress_adapter,"),
        ("Path(runner.__file__)", "Path(ingress_adapter.__file__)"),
    ]
...
def run_tier_d() -> dict[str, int]:
    """Tier D — Wire surface (producer/recipient -> source/destination). Envelope v2."""
    counts = {"producer_to_source": 0, "recipient_to_destination": 0}
