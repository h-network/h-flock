...
|---|---|---|---|---|
| 1 | `.append()` called on the `set` that `list_windows` returns — the `__init__` placeholder path raises `AttributeError` | `tmuxhost/host.py:201`, `tmux/ops.py:56` | claude | ✅ |
...
| 9 | The session door corrupts non-ASCII terminal output | `session/control.py:193-203`, `session/app.py:159-164` | codex | — |
| 10 | One slow viewer grows the session process without bound | `session/control.py:40-45`, `session/app.py:159-167` | **both** | — |
| 11 | The SSE endpoints do blocking Redis I/O on the event loop | `api/app.py:516`, `:659`, `:443` | claude | — |
| 12 | One malformed roster row makes `/board` return `404` for the **entire tenant** | `api/app.py:705-712`, `bus/roster.py:6-8` | claude | — |
