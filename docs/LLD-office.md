...
⚠ **`main()` scopes `FLOCK_LOG_QUIET` to its own run** (`cli.py:1096`), setting
...
⚠ **`send`, `send-file` and `add` do check** (`is_member`, `cli.py:150`, `:265`, `:815`) before
...
- `--profile` is checked against `available_profiles()` when the registry
  exists (`cli.py:539`) — the same registry `profiles` reads — refusing with
...
`_ticket()` (`cli.py:594`) normalizes whatever JSON sits in a list entry into:
...
board entry into a command that assumes a shape. `_serialized()` (`cli.py:629`)
...
⚠ **`held_ts`, set by `hold` (`cli.py:783`), is the youngest of the four
...
--related <id>[,<id>...]` (`cli.py:809`) splits on comma, strips, and dedupes
...
`_select()` (`cli.py:639`) is shared by `take` (with an id), `done`, `cancel`,
...
(more than one ticket sharing that prefix) by name. `_remove()` (`cli.py:655`)
...
| `take` | `todo`, `hold` (by id) | `doing` | refuses when `doing` is non-empty (`cli.py:734`) — checked, not emergent; no id pops FIFO from `todo` |
| `done` | `doing` | `done`, `status: "done"` | |
| `cancel` | `doing` | `done`, `status: "cancelled"` | shares `_finish_command` with `done` (`cli.py:757`) |
...
- `_log_task()` (`cli.py:660`) calls `log_record("office", event, …)`, which
...
scale to `list --all` across a whole tenant. `_ticket_line()` (`cli.py:681`)
...
under**, not a duration score (`_ticket_age`, `cli.py:671`):
...
board, just `git`. `_clone_agents()` (`cli.py:844`) selects live `tmux`
...
than at the local source (`_clone_to_all_command`, `cli.py:915‥926`; the
`remote set-url` itself is `_git_clone`, `cli.py:874`) — so the network cost
...
`cli.py:1113`), which delegates to `office cloneToAll` rather than
...
subclass carrying a user-facing message. `_run()` (`cli.py:1128`) is the single
