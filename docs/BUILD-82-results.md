...
- `delivery.markers` in `src/flock/port/openers.py` has a documented safety
  ...
  markers. `_correlate_delivery` reads the bounded stream window via `xrange`
  and respects the 500 safety bound.
...
## TEST SIGN-OFF — full repository gate
    ...
    claim            ActivityTailer extracts 4 usage buckets, dedupes request IDs per agent via atomic Lua claim/emit, correlates delivery markers within bounded 500 maxlen, prices via longest prefix with unpriced flags and fail-loud config, and office usage reads real RESP xrange and formats summaries
    source sha       fca4d65c1056586be9e41939b7e7d993a322dc0c
    artefact         COMMIT
    ...
    population       432 tests and 5 subtests; all repository tests collected (real redis-server Lua atomic claim test RAN, mandatory fail if unavailable, 0 skipped)
...
Property mutation: mutated `claude-opus-4` cache_read pricing rate from 1.50 to 0.00 in `container/config/pricing.json`.
    ...
    evidence         /tmp/build82-control1.log sha256 644cae4de1013a33223dc89452f6540f48c2c07fbd5f20f917f2db88df594e2a
...
Property mutation: disabled `SISMEMBER` in `_EMIT_USAGE_LUA` and in-memory `_seen_requests` check in `ActivityTailer._emit_usage`.
    ...
    evidence         /tmp/build82-control2.log sha256 bb22cb98f0f7224d14885b6b208299e61b4004c684b1c6c58368840e3f5d4339
...
Property mutation: changed missing model pricing to return `(0.00, True)` instead of `(None, False)`.
    ...
    evidence         /tmp/build82-control3.log sha256 61183ba14b940b0b336d7d0dd9543ca9ea3683545692f0790bfd41b4e6b040f5
...
Property mutation: zeroed out `cache_read` and `cache_write` in `calculate_cost`.
    ...
    evidence         /tmp/build82-control4.log sha256 f3980a564eda33248f55405babd0f94c52830262e0f45fe25fb338de082c3afb
...
Property mutation: disabled `_attributed_markers` tracking so every subsequent usage record reused the preceding marker.
    ...
    evidence         /tmp/build82-control5.log sha256 3656445300f895f3ecac63253fecc97bebb0b19cfd20e0de27db2676eb66385e
...
Property mutation: made `_seen_requests` a single global set across all agents.
    ...
    evidence         /tmp/build82-control6.log sha256 ffa2580a5de3295ed7d963c1786c7b597c88de58deb86515b55f8b800460f838
...
Property mutation: marked request as seen in memory even when Redis emission failed.
    ...
    evidence         /tmp/build82-control7.log sha256 c395021837f8a312bfac264897bbcd8e0c653ac5847e3940cfaee429352ed3df
...
Property mutation: swallowed `FileNotFoundError` in `load_pricing` to silently fall back.
    ...
    evidence         /tmp/build82-control8.log sha256 ad7281128c8008ac096c09cfc5a67b4da9f6a65b4e8d1af1034bd20959a983e6
...
Property mutation: renamed `xrange` to `_disabled_xrange` in `src/flock/bus/resp.py`.
    ...
    evidence         /tmp/build82-control9.log sha256 72bd5220e55eb16e4c51a666f356c6acf8f6e4166c2e70a0d0754db9e8433ee0
...
Property mutation: removed `redis.call("SADD", seen_key, request_id)` from `_EMIT_USAGE_LUA`.
    ...
    evidence         /tmp/build82-control10.log sha256 bc7e8d941100f3cb8122e414f44ee42a76689ebc60a04aa28d8abbb8c01c7e67
