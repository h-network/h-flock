...
| | |
|---|---|
| ~~**profile logins**~~ **— DECIDED 2026-08-26, not solved but settled.** Two paths: mint a `CLAUDE_OAUTH_TOKEN_<PROFILE>` where the CLI supports it (claude — see the `CLAUDE_CODE_OAUTH_TOKEN` row below), or log in interactively once and persist the credential with `seed-home.sh out`/`in` (codex, agy). The human step doesn't disappear; it happens at most once per account rather than once per rebuild. The original: one interactive login per account. Not buildable — a person has to do it. ⚠ **Checked: nobody has solved this.** NVIDIA OpenShell's own tutorial says you authenticate with your own account in a browser, and trust the workspace when prompted |
| ~~**local model: long-context behaviour unknown**~~ **— DECIDED 2026-08-26, not pursuing.** Behaviour varies too much per model to generalise into an h-flock-level claim; not worth chasing as a framework concern. The original: every test was a short turn against a 65k window. Nothing says what a local agent does when it fills |
| ~~**security: what is left after build 36**~~ **— AMENDED 2026-08-27, scoped rather than closed.** 2026-08-26 closed this as "not building CORS/per-client tokens" on the reasoning that the trust boundary is the container, not identity inside it. That reasoning holds intra-tenant and for a loopback-only door — but a *published* api door (`API_PUBLISH=1`) is reachable from outside that boundary entirely: any caller with the one shared bearer token, not a colleague sharing `sudo`. The gap the original row named is real there: `as` is a self-declared field nobody verifies, and the token has no per-client revocation. **Scope: per-client HMAC (`kid`/`sig`) and CORS, required only when published, off for loopback-only.** Rotation (implied by `kid`'s date suffix) still needs an answer. Audit logging remains the right tool for the intra-tenant case; this amendment is about the case that isn't intra-tenant. The original: ⚠ **The boundaries are done** — TLS on both doors with a refusal to serve a non-loopback bind without it, `source` stamped from its egress queue, and a tenant that will not start with a widened Redis bind and no password. What remains is **CORS and per-client tokens** on the api door. ⚠ **Nothing here isolates agents from each other**, deliberately: h-flock is a development office, agents are colleagues who were hired. HMAC envelopes, a brokered `office`, one OS user per window — that is a service executing work for callers it does not trust, a different product |
| **the console cannot reach TLS doors** ⚠ *found by testing TLS, not by reading* | `clients/web/server.py` proxies for the browser, and its own client is plaintext-only: the WebSocket proxy opens a bare `socket.create_connection` (so terminals break against **any** cert, valid or not), and the REST proxy uses the default verifying context with no CA or insecure option. ⚠ **A supported configuration that breaks the shipped demo is a defect, not a missing feature.** Recorded rather than fixed only because nobody has picked it up yet — the `clients/` freeze this used to be blocked behind was reversed 2026-08-29. Roughly 30 lines in one file: ssl-wrap the socket, pass a context, add the option. Until then TLS belongs in a reverse proxy in front of loopback-published doors (`LLD-container` §3) |
...
| **seeded credentials do not survive `--force-recreate`** ⚠ *half of this row was already false* | They live in the container filesystem by design — never baked, never a volume — so recreating deletes them, and `seed-home.sh out` before a rebuild is still the only way back. ⚠ **But `seed-home check` does NOT lie any more.** This row said it reported "logged in" by testing the *host's* staging copy; it runs `docker exec` and tests paths **inside** the container, per profile, including `.claude-<p>/.credentials.json` and `.codex-<p>/auth.json`. Corrected 2026-08-23 by reading it rather than trusting the row. ⚠ **`out` covers profiled directories too** — `CRED_PATHS` for the three unprofiled plus a glob over `.claude-*` and `.codex-*`. ⚠ **agy has NO per-profile support anywhere**: `window_env` sets `CLAUDE_CONFIG_DIR` and `CODEX_HOME` for a profile and nothing for agy, `ensure_agy_project_trusted` takes no profile, and `check` sets `agy_path=""` for any non-default account. **One agy account per tenant**, however many accounts are configured, and nothing says so at setup. ⚠ **What remains**: the round trip itself — `out`, rebuild, `in` — is the manual step, and the OAuth token removes it for claude only. ⚠ **UPDATED 2026-08-29**: this is now the whole shape of an "official upgrade path," not just credentials. Three independent drafts (api, bus, acceptance — see ticket history) converged on the same sequence: stop the old container (not `rm`) → rename it out of the way (a plain `docker compose up` on a bumped image tag recreates in place otherwise, destroying the rollback artifact — found by acceptance, not assumed) → stage state to a host directory, matching `container/home/`'s own precedent over inventing a volume-based mechanism → create the new container, custody's named volume reattaches automatically by project name, no copy needed → restore staged state → re-hire every agent from a pre-upgrade roster snapshot, letting `has_session_history` auto-resume pick the copied history back up on its own → verify per-agent with `seed-home.sh check` (real auth, not file presence) plus a real message round-trip, not just "process up" → keep the renamed old container until an explicit finalize. **In scope, confirmed by acceptance's read of the code, not by assumption:** `/workdir/<agent>` (git trees, uncommitted files) and each CLI's session history, not just credential files — losing those is a bigger cost than losing a login. **Board policy, still open:** refuse the upgrade by default if any agent's `todo`/`doing`/`hold` is non-empty, matching `send_refused`'s shape, rather than silently dropping tickets. Not yet routed as an implementation ticket |
| **`office swap <agent> --cli <x>`** ⚠ *operator's idea, and mostly already built* | The CLI is a Redis value (`agent:<name>:launch`) and `tmuxhost` rebuilds a missing window from it, so replacing the process while keeping the name, board, queues and workdir needs no new machinery. ⚠ **Not stop-then-start:** `StopAgent` destroys an api client's unread mailbox (audit F6). Open questions: drain or discard the ingress, what presence reads during the gap, and what happens to a ticket already in `doing`. Costs the agent's memory, which is acceptable. ⚠ **CONFIRMED LIVE 2026-08-29**: the underlying retire-then-rehire mechanism was tested end to end (`office letGo testbed`, then `office hire testbed --cli codex --resume`) — full prior conversation history came back verbatim, verified by capturing the pane directly, not by trusting the command's own exit code. Still two manual commands, not the single `swap` this row asks for, and the operator has to already know which CLI/profile to specify — nothing in `office profiles`/`office peers -v` remembers a retired agent's prior config, so that knowledge lived only in the operator's own memory during the test. **Related, not-yet-built need surfaced by this**: a small durable record (agent name → cli/profile/provider/hired_ts/retired_ts) that survives `purge_agent`, which today clears exactly this from `AGENT_STATE_RESOURCES` on retirement. Would need its own tenant-scoped resource, not a per-agent one, plus a new `office` read command. Not yet routed as an implementation ticket |
...
| ~~**`docs/LLD-port-tmux.md`, line 150 at the time, still described the pre-build-92 vocabulary**~~ **— SHIPPED in build 95** | Corrected in `tmux`'s own lane, alongside the swallowed-kick fix. ⚠ **Citation note, 2026-08-29**: the 2026-08-29 LLD-port-tmux/LLD-port-delivery split moved this content out of `LLD-port-tmux.md` entirely; the correct current statement — `board_write_confirmed` on success, `board_write_unknown` on an exception, `board_write_failed` only for a provably-invalid returned length — now lives at `docs/CONTRACTS.md:753-756`. The original: | It says every exception logs `board_write_failed`; the exception path now logs `board_write_unknown`, and `board_write_failed` survives only for a **returned** invalid depth. ⚠ **`bus` found it while editing a different file and reported rather than fixed it**, which is correct — contradictions inside the file you are editing are in scope, contradictions in someone else's are a row. ⚠ **tmux's lane owns it.** Small, and it is a documented falsehood until done |
...
| **a revoked OAuth token is invisible to the watchdog** ⚠ *the cost of fixing the false `absent` alert, and it is mine not a lane's* ⚠ *HIGHER PRIORITY as of 2026-08-27* | Build 91 stops `watchdog/service.py` alerting `absent` forever for a token-authenticated agent — correct, because that agent has no credentials file **by design**. ⚠ **But it skips the check entirely rather than checking something else**, so an agent whose token was **revoked or expired** is dead and the watchdog is silent. That is precisely the case the check exists for. ⚠ **Not fixable locally**: a token cannot be validated without an API call, and presence in the window environment is the only signal available offline. So the honest options are a periodic authenticated probe, or leaving it and saying so. **It is said** — in `BUILD-91-results` (deleted in `6dd3f1f`) and at the `continue` itself. ⚠ **Recorded because neither the author nor either verifier raised it**; three people read that diff and the silence was invisible to all of them, which is the argument for writing limits down rather than trusting review to surface them. ⚠ **Raised, not lowered, 2026-08-27**: the `CLAUDE_CODE_OAUTH_TOKEN` row below is now DECIDED as the *preferred* path for claude credentials — every account moving to token auth widens this exact blind spot rather than shrinking it |
...
| ~~**`office status` says `unknown` for an agy agent while the column beside it says why**~~ **— ANSWERED 2026-08-27: presence was NOT genuinely underivable, and the open question this row existed to hold is closed** | The open question the acceptance seat flagged and declined to decide — *is presence genuinely underivable for agy* — is answered: no. `~/.gemini/antigravity-cli/history.jsonl` is a real, live, one-line-per-submitted-input log (`{display, timestamp, workspace, conversationId}`), and `ActivityTailer` now tails it (`watchdog/activity.py`'s `_agy_events`, filtered per line by `workspace` since the file is shared across every agy agent on the host). `PresenceSampler._tailable()` and `office/cli.py`'s status-row renderer both now treat agy exactly like claude/codex — no more special case, no more `not collected (agy)`. Verified live against a real hired agy agent: real `input` events landed in its activity stream with correct per-line attribution and timestamps converted from history.jsonl's millisecond epoch. ⚠ **Not yet visible in THIS tenant's own `office status`** — the running watchdog/office processes are an installed package several commits behind `main` (deploy-lag, tracked separately), so they still run the old code and print the old `unknown` / `not collected (agy)` until redeployed. The fix is real and proven against live data; only its rollout to this office is pending. The original: | After build 88 the **activity** column correctly reads `not measurable (agy)`. The **status** column still reads `unknown`, where claude agents read `idle`. ⚠ **That is one word doing two jobs one column apart** — *"we cannot determine this"* and *"we know, permanently, that this cannot be measured"* — which is exactly the confusion build 88 removed from the activity column and left in place next to it. ⚠ **Open question, not a verdict**: whether presence is genuinely underivable for agy, given a tmux pane exists either way, has not been established. The seat flagged it and declined to decide it. ⚠ **Checked 2026-08-27**: `office/cli.py:295` prints `not collected (agy)` (not `not measurable`, a wording drift from build 88's own row), and it appears in the same `office status` line as the bare `unknown` status — so read together the row is self-explanatory in practice today, even though the `status` field alone still carries no reason. The underlying open question (is presence genuinely underivable for agy) is unchanged and still undecided |
| **an alert you can clear** ⚠ *asked for by the operator* | Alerts are an append-only stream with no acknowledgement. Clearing must be keyed by **cursor** — one instance — so it can never become "mute this kind". Spec: `SPRINTS.md` Sprint 4 §1 (originally `BUILD-38-durable.md` §1, deleted in `6dd3f1f`) |
| ~~**credential alerts never clear**~~ **— DECIDED 2026-08-26, no code change.** Only ever reachable for a file-based claude login; a token-authenticated account skips the file check in `_credential_accounts()` entirely (see the `CLAUDE_CODE_OAUTH_TOKEN` row below). Needs a code comment at that skip point explaining why, not a fix — separate small ticket. The original: Measured: `status=absent` raised at `01:00:42Z`, login completed at `01:07Z`, nothing ever retracted it, so the console correctly rendered a fact that had been false for an hour. ⚠ **It was only ever tested firing.** Originally `BUILD-38-durable.md` §2, deleted in `6dd3f1f`; kept for the measurement in `SPRINTS.md` Sprint 4 §2 |
| **the permission mode lives only in argv** ⚠ *probably closed, and the trigger was never reproduced* | ⚠ **The base image now ships `skipDangerousModePermissionPrompt: true` in `~/.claude/settings.json`**, and a file survives the CLI self-re-exec that argv does not — which is the exact mechanism this row describes. h-flock inherits it since 2026-08-23 and no longer ships its own copy. ⚠ **Not marked closed**, because the original trigger was never reproduced (*"a forced resize does not reproduce it"*), so there is nothing to test the fix against. If an agent is seen asking for permission again, this row is why. The original: | A hired agent starts as `claude --dangerously-skip-permissions …` (verified at +4s/+8s/+12s) and was later seen as bare `claude` carrying `CLAUDE_CODE_RELAUNCH_*`: the CLI re-executed itself and the flag went with it, leaving the agent asking for permission. ⚠ **The trigger is unknown** — a forced resize does not reproduce it. Originally `BUILD-38-durable.md` §3, deleted in `6dd3f1f` and not otherwise recovered — `git show 6dd3f1f~1:docs/BUILD-38-durable.md` has it if this is revisited |
| **console conversation needs `--audit-log`** | Outbound messages are rebuilt from the audit log, so without that flag every refresh looks like data loss. Agent replies survive; yours do not. `SPRINTS.md` Sprint 4 §4 (originally `BUILD-38-durable.md` §4, deleted in `6dd3f1f`), and a failing flow in `clients/web/flow-check.py`. ⚠ **Sequenced 2026-08-26: after, not before.** Telegram and the REST API get dogfooded properly first; webapp/console work — including this — waits until then |
...
| **the task board has no push, so a lead counts by hand** ⚠ *the coordination role is by design; the bookkeeping is not* | ⚠ **A maintainer coordinating SMEs across separate modules is the intended shape**, and peer-to-peer messaging already exists — that is not the gap. The gap is mechanical: a ticket lands with **no notification**, so `AGENTS.md` instructs a manual `sendMessage` doorbell after every `jira add`, and in a real run the lead also hand-counted each agent's "N/10" message budget. ⚠ **Phase 2 of that run had no ticket at all**, so the debate was invisible on the board and unauditable afterwards. Wanted: assignment delivers itself, and phase work can carry a ticket per participant. ⚠ **Partially addressed 2026-08-27**: watchdog's unpicked-ticket alert (`docs/LLD-watchdog.md` §2b, merged) means a ticket sitting unnoticed in `todo` now surfaces to the lead on its own — "assignment delivers itself" for the notification half. The lead still hand-counts message budgets and per-participant phase tickets; that half is untouched. Candidate area for the board-enhancement work discussed 2026-08-27 |
| ~~**an agent can address a participant it cannot discover**~~ **— DECIDED AND RECORDED, 2026-08-28: `office peers -i`** | `src/flock/office/cli.py:173` filters `office peers` to `port_type == "tmux"`, so an api client is **omitted entirely** rather than shown without a framework. ⚠ **Still deliberate and still gated** — `container/plumbing-check.sh:150`/`:151` (*peers hides client*, *peers really hides*) still invoke plain `office peers` with no flags and still pass unchanged. ⚠ **The reasoning is now written down rather than assumed**: a real incident (an agent that received `[message from telegram]`, couldn't find it in `office peers`, and answered in plain terminal text instead of routing the reply) showed that "addressable and undiscoverable" is a real cost, not a hypothetical one — so the gap closed is discoverability, not the hiding itself. `office peers -i`/`--interfaces` (`LLD-office.md` §4, `CONTRACTS.md` §5) prints `api`/`control` roster members on a second, explicitly labeled line, opt-in and never merged into the peer list — bare `peers` and `peers -v` are byte-for-byte unchanged, so both acceptance gates and the "client is not a colleague" reasoning both hold. Membership in the roster is itself the sanctioning act (every `api`/`control` row exists because a `StartAgent` call put it there), so `-i` reads existing state rather than inventing an "allowed" flag |
...
| **presence and cost are not comparable across CLIs** ⚠ *presence half CLOSED 2026-08-27 — cost remains genuinely open, and read the four rows above before working this* | ⚠ **codex is fixed**: the model comes from `turn_context` per turn, so rows price instead of reading `unpriced`, and per-turn `last_token_usage` is used rather than cumulative `total`. ⚠ **agy's PRESENCE is now fixed, not just answered** — `history.jsonl` gives real `input` activity, so `office status` reads `idle`/`working` for agy the same as any other CLI (row above, this file). ⚠ **agy's COST remains answered, not fixed, and this time it is checked rather than assumed**: `history.jsonl` carries no token counts, and `~/.gemini/antigravity-cli/conversations/*.db` — the only other candidate — is opaque protobuf with no readable schema. `office usage` correctly keeps naming agy **not collected**; there is nothing to parse. ⚠ **What is still open has its own row above**: `rate_limits` has never run against a live codex agent. The original: | ⚠ **Measured in one tenant**: `office status` reported an agy agent `unknown — no activity feed` **while it was actively messaging** (`activity.py` parses claude and codex formats only); codex reported `model: unknown` so every codex row prices as `unpriced`, indistinguishable from a free local model; and agy contributed **zero** usage records despite being the most talkative peer. ⚠ **So the cost table invites a comparison it cannot support.** Either normalise per-CLI adapters or state plainly in the output which agents are not measurable |
...
| **a held ticket has no way back** ⚠ *found live, 2026-08-29* | `office hold` moves a ticket out of `doing`; nothing moves it back. `office --help` has no unhold/resume-ticket verb, and `done`/`hold`/`delete` are all scoped to the calling agent's own board with no `-a` — the ticket's owner is the only one who can act on it, and even they cannot reopen a held one directly. ⚠ **Measured**: `office done <id>` on a held ticket returns `no task matches id` — `done` does not search `hold`. The only path found live was `office delete <id>`, which removes the ticket outright rather than resolving it, and was fine for that specific case only because the ticket was informational (a now-superseded open question), not real work to finish. **A real `doing`-shaped held ticket has no non-destructive way back to `todo`/`doing` today** |
| **office performance table** ⚠ *operator's idea, 2026-08-29* | A single table of what's fast and what's slow, the good and the bad, rather than performance numbers scattered across build docs and rows here as they're found. Existing data points to pull together, not re-measure: the ~659–911ms per-`flock.port`-kick process-startup cost (`LLD-bus-and-switch.md`, the reason for the hand-rolled RESP client), the 6.5/s (lab) vs 853/s (h-oracle) throughput spread noted above, whatever the registry/lazy-import work changed or didn't. Not scoped further than that — a real pass would need deciding what counts as "the good" as well as the bad, and re-measuring rather than trusting old numbers, per this file's own repeated warning about rows rotting |
| ~~**agents ack-loop on pure acknowledgments**~~ **— BOTH HALVES SHIPPED, 2026-08-29** | A three-way introduction between `mod-h-openshell`, `codex-nemostack`, and `sme-openshell` produced two simultaneous ping-pong threads — each side kept replying to the other's closing "got it"/"thanks" with another acknowledgment, with nothing substantive left to say. Confirmed live and stopped by the architect manually. **Detector**: `bus.send()` tracks a directed ack-shaped-reply streak per tmux peer edge, no message content ever stored (`CONTRACTS.md`); the watchdog messages the lead — never either looping agent — once both directions independently cross a threshold, the same "reports, never repairs" shape as `LLD-watchdog.md` §2a-d. **Wording**: `flock.tmux.ops.generate_agents_md` now distinguishes "always reply via `office send`" (a substantive message must actually reach its recipient) from "a closing ack doesn't need another ack back" (replying to an acknowledgment with another acknowledgment is a loop, not courtesy) — landed by architect, own instruction class as `CLAUDE.md` itself |
