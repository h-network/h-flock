...
| **alert before claude expires, and when a login is missing** | the watchdog already warns on claude's refresh-token expiry, but a **missing or unreadable credential file is silently skipped** (`watchdog/service.py`, `except (OSError, json.JSONDecodeError): continue`). The lab tenant sat at `Not logged in` for hours with the watchdog running and nothing said anything |
