...
| **PauseAgent/ResumeAgent blindly use tmux operations for non-tmux participants** ⚠ *found by the 2026-08-29 docs sweep* | `StartAgent` now admits `tmux`, `api`, and `openshell`, but `pause_agent` and `resume_agent` never read the roster port_type: after publishing/removing the generic `paused` marker they always call the injected tmux window interrupt/resume functions. For an OpenShell or API participant, pause therefore changes desired state and then reports an incomplete actual action against a nonexistent window; resume clears the marker, fails the nonexistent window resume, and never reaches its queued-ingress kicks. This is a functional gap, not prose drift. **Two real choices need a decision:** explicitly reject PauseAgent/ResumeAgent unless port_type is `tmux`, or define and implement port-specific pause/resume actual-state semantics (including how an OpenShell headless sandbox is “interrupted” between one-shot deliveries). Do not merely edit the generic lifecycle docs to bless the current partial mutation. |
