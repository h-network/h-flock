...
| **command naming is inconsistent** ⚠ *minor, recorded because an agent tripped on it* | `letGo` and `cloneToAll` are camelCase while every other verb is lowercase, and shell CLIs conventionally use kebab-case. Aliases would cost nothing and remove a class of guess |
