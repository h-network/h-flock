...
| ~~**agents ack-loop on pure acknowledgments**~~ **— detector SHIPPED, CLAUDE.md wording still open** | A three-way introduction between `mod-h-openshell`, `codex-nemostack`, and `sme-openshell` produced two simultaneous ping-pong threads — each side kept replying to the other's closing "got it"/"thanks" with another acknowledgment, with nothing substantive left to say. Confirmed live and stopped by the architect manually. **Detector shipped, 2026-08-29** (`LLD-watchdog.md` §2e): `bus.send()` tracks a directed ack-shaped-reply streak per tmux peer edge, no message content ever stored (`CONTRACTS.md`); the watchdog messages the lead — never either looping agent — once both directions independently cross a threshold, the same "reports, never repairs" shape as §2a-d. ⚠ **Still open**: the office-wide instruction text (`flock.tmux.ops.generate_agents_md`) still doesn't distinguish "always reply via `office send`" from "a closing ack doesn't need another ack back" — architect owns that sentence since it's the same office-wide instruction class as `CLAUDE.md` itself, landing separately |
