...
| ~~**security: what is left after build 36**~~ **— DECIDED 2026-08-26, not building CORS/per-client tokens.** The trust boundary is the container, not identity inside it — access control that doesn't match that model is false security, not defense in depth. Audit logging (the custody chain) is the honest tool for this trust model; access control is not. The original: ⚠ **The boundaries are done** — TLS on both doors with a refusal to serve a non-loopback bind without it, `source` stamped from its egress queue, and a tenant that will not start with a widened Redis bind and no password. What remains is **CORS and per-client tokens** on the api door. ⚠ **Nothing here isolates agents from each other**, deliberately: h-flock is a development office, agents are colleagues who were hired. HMAC envelopes, a brokered `office`, one OS user per window — that is a service executing work for callers it does not trust, a different product |
