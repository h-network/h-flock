...
| **an agent cannot tell what its peers are** ⚠ *and capability differs by CLI* | `office peers` returns names. Nothing says which framework a peer runs — yet claude, codex and agy differ in what they can do, and an agy peer cannot be pointed at a local model or priced at all. ⚠ **Every agent in the 2026-08-23 run reasoned about peers without knowing what they were.** Raised by the agy agent, which is the one that most needed it. `office peers -v` showing framework, profile and current task |
