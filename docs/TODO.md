...
| **an agent's message content gets executed by its own shell** ⚠ *observed live* | Agents compose `office send -a x "..."` through a Bash tool, so the **message body is parsed by the shell**. Measured on a four-agent run: an agent discussing hypothetical ISA instructions produced `/bin/bash: line 14: VERIFIED-MODE: command not found` and `BUDGET-ALLOCATE: command not found` — words from its own prose executed as commands. Harmless there; the same mechanism with a code block containing backticks, `$(...)` or `rm` is not. ⚠ **It also silently drops the message** — a send that dies in the shell never reaches the recipient, and the conversation thread stalls with no error anywhere. Needs a way to pass a body that is never shell-parsed: stdin, a file, or `office send -a x --stdin` |
