...
| **`accept.sh` needs a playwright venv on the lab** ⚠ *half closed, and the remaining half is one line in `BUILD-CONVENTION`* | ⚠ **`accept.sh` handles it and says so loudly**: `accept.sh:190` tests for playwright, and without it prints *"console FLOWS WERE NOT CHECKED"* and *"This run is incomplete"* rather than staying silent. ⚠ **But `BUILD-CONVENTION` still never mentions it**, so an operator standing up a new lab host has no way to know. ⚠ **And see the row below** — the warning is prose, not status. The original: | The console flows in acceptance need `PATH=~/pw-venv/bin:$PATH` on `h-lab@172.16.0.14` or they skip — and **a skip reads as green**. This is why acceptance ran "clean" for weeks without ever exercising the console. ⚠ **Belongs in `BUILD-CONVENTION`**, next to the two hosts |
