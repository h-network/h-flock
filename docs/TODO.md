...
| **agents ack-loop on pure acknowledgments** ⚠ *found live, 2026-08-29* | A three-way introduction between `mod-h-openshell`, `codex-nemostack`, and `sme-openshell` produced two simultaneous ping-pong threads — `mod-h-openshell` kept replying to each closing "got it"/"thanks" with another acknowledgment, and the other side kept doing the same back, with nothing substantive left to say. Confirmed live and stopped by the architect: told all three to stop replying to pure acknowledgments. **The office rule needs a sharper reading**: "always reply via `office send`" means a substantive message must actually reach the recipient (a real reply written only in your own pane never arrives) — it does not mean every closing ack requires another ack back. Nothing in `CLAUDE.md` or here currently distinguishes those two cases, which is presumably why three separate agents converged on the same loop independently. **Routed to `watchdog`, 2026-08-29**: manual stop by the architect isn't a fix, and watchdog is the lane that already watches agent liveness/behaviour — needs a real detector for ack-loop patterns (or a sharper `CLAUDE.md` rule plus something that catches agents who don't follow it) rather than relying on a human/lead noticing and intervening each time |
