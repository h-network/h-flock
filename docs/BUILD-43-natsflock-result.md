...
Run on the lab with nats:2.11-alpine and nats-py 2.11.0:
    ...
    phase=publish-and-leave-no-adapter-running
    ...
    phase=start-one-explicit-adapter-process
...
The server retained the message correctly while no adapter existed. It did not
and cannot start flock.adapter. A pull consumer needs a client fetch loop; a
push consumer needs a continuously subscribed client. Either is a resident
process. Polling pending counts from tmuxhost would merely move the router into
tmuxhost and couple window reconciliation to transport. A separate dispatcher
would be the router under a new name.

## Five-record criterion

The result fails the criterion. The publish edge can emit sent, and the adapter
...
Those two names could be printed by the adapter after fetch, but that would
collapse router custody, broker forwarding and adapter receipt into one edge.
...
adapter. That is at least the two responsibilities of the existing small router,
...
That choice exposes the mismatch. If the adapter is not resident, something
...
server is retained while no adapter exists. That is real, but it does not close
...
stop when the adapter lifecycle needs a daemon or the trace needs a bespoke
