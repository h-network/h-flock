...
`LLD-watchdog` §2a), a ticket sitting unpicked in `todo` past
`WATCHDOG_TODO_ALERT_SEC` (default 5 minutes, `LLD-watchdog` §2b), a ticket
parked on `hold` past `WATCHDOG_HOLD_ALERT_SEC` (default 60 minutes,
`LLD-watchdog` §2c), or a client message an agent has left unanswered past
`WATCHDOG_UNREPLIED_ALERT_SEC` (default 60 seconds, `LLD-watchdog` §2d), is
pasted straight into the lead's pane — `office send`'s path, not the alerts
stream. Every reason above for never messaging an agent is about the *agent
doing (or not yet doing, or having paused) the work*: telling it something is
wrong invites it to "fix" the report instead of the problem, and the paste
itself would erase the evidence it just delivered. **None of that applies to
the lead**, because the lead is not the thing being observed — the ticket's
or message's agent is. A message to the lead cannot reset the ticket's own
silence timer, make an unpicked ticket look picked, make a held one look
resumed, or make an unanswered client message look answered; only a message
to the *named agent* could do any of those, and none of the four alerts is
ever sent there. So the "reports, never repairs" boundary holds exactly as
before: the watchdog still does not tell the affected agent anything, still
does not repair, and still only widens what a human can see.
