...
- retiring `host` deletes the tenant's control endpoint, and an empty roster
  then spins the router against Redis — one chain, found by cross-checking two
  separate findings (F2+F3)
- two codex agents without profiles share a session directory, so the router
...
- **the broadcast fan-out is atomic.** `service.py:78` calls `pipeline()` with no
  ...
  `LLD-bus-and-router.md:637` says "pipelined, not atomic". ✅ **verified**
- `LLD-tmux-host` documents two bugs that were already removed
- "the router does not rewrite the envelope" is stated absolutely in one place
