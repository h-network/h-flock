...
⚠ **Launch, Profile, and Cause State Ordering:** `start_agent` (`flock.control.openers`) writes the `launch` (`pod:<pod>:tenant:<tenant>:agent:<name>:launch`), `profile`, and fresh-hire `window.cause` keys to Redis *before* writing roster membership (`r.hset(roster_key, agent, agent_port_type)`). `tmuxhost` reconciles windows as soon as the agent row appears in the roster; writing any of this state after roster membership creates a race where tmuxhost can build a window with the default CLI or wrong account, or emit `window_created` without the hire's correlation. `window.cause` is one-shot: successful creation consumes it into the event, while an already-present window discards it without attribution so a later recovery cannot borrow a stale cause.
