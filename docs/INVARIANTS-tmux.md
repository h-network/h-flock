...
| tmux_reconciler is the sole creator after `StartAgent`; desired launch/profile/provider state is visible before the window. | A new window appears with a command or environment different from the Redis launch/profile/provider keys that preceded it. |
...
| A verifiable tmux paste records both the watchdog marker and the usage-correlation marker before delivery. | A `claude`, `codex`, or `agy` `Message`/`Command`/`Attachment` paste has no same-stream-id entry in either `pending.verify` or `delivery.markers`, or either marker is timestamped after the paste. |
