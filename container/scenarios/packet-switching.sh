...
set -uo pipefail
. "$(dirname "$0")/_lib.sh"
MODE=steady; COUNT=${COUNT:-10}; ROUNDS=${ROUNDS:-10}; WORK=""; RECONCILE_ONLY=""; EXPECTED=""; RUN_ID="${RUN_ID:-$(date +%s)-$$}"
while [ "$#" -gt 0 ]; do
  case "$1" in
    --mode) MODE="$2"; shift 2;; --count) COUNT="$2"; shift 2;;
    --rounds) ROUNDS="$2"; shift 2;; --work) WORK="$2"; shift 2;;
    --reconcile-only) RECONCILE_ONLY="$2"; shift 2;; --expected) EXPECTED="$2"; shift 2;;
    -h|--help) echo "packet-switching.sh [--mode steady|burst] [--count N] [--rounds N] [--work DIR] [--reconcile-only DIR --expected N]"; exit 0;;
...
d = pathlib.Path(sys.argv[1]); expected = int(sys.argv[2]); prefix = sys.argv[3] if len(sys.argv) > 3 else "bench-"; log = d / "custody.log"; sent = {}; opened = collections.Counter(); popped = {}; forwarded = {}; stages = collections.Counter(); ignored = 0
for line in log.read_text(errors="replace").splitlines():
    ...
    if not sid: continue
    own = any(str(rec.get(field, "")).startswith(prefix) for field in ("source", "destination"))
    if not own:
        ignored += 1
        continue
...
print(f"PACKET_SCOPE source_or_destination_prefix={prefix} ignored_out_of_scope={ignored}")
...
if [ -n "$RECONCILE_ONLY" ]; then
  ...
  # Fixture logs predate live run IDs; keep their documented bench- universe.
  # A fixture without --expected exercises reconciliation only and must not be
  # rejected as incomplete. Live runs always pass their harness-side count.
  judge "$RECONCILE_ONLY" "${EXPECTED:-0}" "${FIXTURE_PREFIX:-bench-}"; rc=$?
  if [ "$rc" -eq 0 ]; then
    echo "RESULT packet-switching pass"
  else
    echo "RESULT packet-switching fail rc=$rc" >&2
  fi
  exit "$rc"
fi
...
PREFIX="bench-${RUN_ID}-"
WORK="${WORK:-/tmp/packet-switching-$TENANT-$RUN_ID}"; mkdir -p "$WORK"
restore_kick() {
  if [ -n "${names:-}" ]; then
...
docker exec "$CONTAINER" redis-cli HSET "pod:${POD:-acme}:tenant:$TENANT:roster" $(printf "${PREFIX}%s api " $(seq 1 "$COUNT")) >/dev/null 2>&1 || incomplete packet-switching roster_seed
names="$(printf "${PREFIX}%s " $(seq 1 "$COUNT") | sed 's/[[:space:]]*$//')"
receiver="python3 /tmp/build114-bench-port.py --pod '${POD:-acme}' --tenant '$TENANT' --count '$COUNT' --prefix '$PREFIX' --idle-exit 8 >>/proc/1/fd/1 2>&1 &"
...
import os, redis
r = redis.Redis.from_url(os.environ.get('REDIS_URL', 'redis://127.0.0.1:6379/0'))
print(sum(r.llen(k) for k in r.scan_iter(match='pod:${POD:-acme}:tenant:${TENANT}:agent:${PREFIX}*:ingress')) +
      sum(r.llen(k) for k in r.scan_iter(match='pod:${POD:-acme}:tenant:${TENANT}:agent:${PREFIX}*:egress')))
...
judge "$WORK" "$((COUNT * ROUNDS))" "$PREFIX"; rc=$?
capture_diagnostics "$rc"
expect "packet reconciliation" 0 "$rc"; finish packet-switching
