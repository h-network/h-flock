...
import json, sys
sid, target=sys.argv[1:3]; opened=dead=parse_failures=0
...
print(opened, dead, parse_failures)
' "$stream_id" "$DESTINATION"
}

deadline=$(( $(date +%s) + DEADLINE_SECONDS ))
opened=0; dead=0; parse_failures=0
while [ "$(date +%s)" -lt "$deadline" ]; do
  read -r opened dead parse_failures <<<"$(custody_counts)"
